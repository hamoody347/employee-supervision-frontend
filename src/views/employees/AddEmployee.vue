<template>
    <div class="bg-white border border-transparent rounded-lg shadow-2xl  p-10">
        <div class="flex place-content-between mb-7 pl-2 border-b-2 pb-4 w-full">
            <span class="font-bold text-4xl text-left ">Add Employee</span>
        </div>
        <form>
            ...
            <div :class="{ 'columns-2 gap-10 mb-4': !json['senior'], 'columns mb-4': json['senior'] }">
                <Input mode="select" label="Employee Role" v-model="json['role']" :options="roles"></Input>
            ...
            <div class="block align-middle text-left border-t-2 pt-4 mt-20">
                ...
                <button
                    class="bg-red-700 hover:bg-red-600 hover:border-red-600 text-white py-2 px-4 min-w-fit border-2 border-red-700 rounded-xl ml-2"
                    @click.prevent="$alert.back">Cancel</button>
            </div>
        </form>
    </div>
</template>
...
export default {
    ...
    methods: {
        submit() {
            ...
            this.$http.post('api/employees', this.json)
                .then((res) => {
                    this.$alert.showSwal({
                        type: "success",
